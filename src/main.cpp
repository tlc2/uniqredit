...
#if defined(NDEBUG)
# error "Uniqredit cannot be compiled without assertions."
#endif
...
CScript DEV_SCRIPT;
const string strMessageMagic = "Uniqredit Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s UNIQ additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("uniqredit-scriptch");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
	...
	miner = CUniqreditAddress(m).ToString().c_str();
		...
		for (size_t i = 0; i < tx.vin.size(); i++){
			...
			for (CScript::const_iterator pc = script.begin(); script.GetOp(pc, opcode, vch);){
				if (opcode == 33){
					...
					spendAddress = CUniqreditAddress(addressRet).ToString().c_str();
