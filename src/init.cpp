 ...
 * between uniqreditd, and uniqredit-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("uniqredit-shutoff");
...
#endif
    GenerateUniqredits(false, 0, Params());
    StopNode();
    StopTorControl();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), UNIQREDIT_CONF_FILENAME));
    if (mode == HMM_UNIQREDITD)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), UNIQREDIT_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/uniqredit/uniqredit>";
    const std::string URL_WEBSITE = "<https://uniqreditcore.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("uniqredit-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that Uniqredit is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Uniqredit version %s\n", FormatFullVersion());
}

/** Initialize uniqredit.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Uniqredit process is using the data directory.
    ...
    GenerateUniqredits(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
