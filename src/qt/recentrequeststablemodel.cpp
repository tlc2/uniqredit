    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return UniqreditUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, UniqreditUnits::separatorNever);
            else
                return UniqreditUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+UniqreditUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
