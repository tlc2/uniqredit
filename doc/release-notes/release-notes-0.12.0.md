Uniqredit Core version 0.12.0 is now available from:

  <https://uniqredit.org/bin/uniqredit-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/Uniqredit-Qt (on Mac) or
uniqreditd/uniqredit-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of Uniqredit Core or other software:
...
pre-0.12 versions of Uniqredit Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Uniqredit Core version 0.11 or
earlier.
...
ECDSA signatures inside Uniqredit transactions now use validation using
[libsecp256k1](https://github.com/uniqredit-core/secp256k1) instead of OpenSSL.
...
(https://github.com/uniqredit/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Uniqredit Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Uniqredit Core was to raise the default minimum
relay fee.

Uniqredit Core 0.12 will have a strict maximum size on the mempool. The
...
Uniqredit Core 0.12 also introduces new default policy limits on the length and
...
Uniqredit Core 0.12 nodes. Uniqredit Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/uniqredit/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Uniqredit Core 0.12 does not yet have support for
...
This allows running uniqreditd without having to do any manual configuration.
...
Uniqredit Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Uniqredit Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Uniqredit Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Uniqredit Core 0.13, and it is at this time undecided whether the
...
Uniqredit Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Uniqredit Core automatically creates a hidden service to listen on, without
manual configuration. Uniqredit Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Uniqredit Core is listening, and
...
Uniqreditd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Uniqredit
Core to use floating fees. By default, Uniqredit Core will use floating
fees.
...
(default: `0.0002` UNIQ/kB).

At all times, Uniqredit Core will cap fees at `-maxtxfee=<x>` (default:
0.10) UNIQ.
Furthermore, Uniqredit Core will never create transactions paying less than
...
`uniqredit.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/uniqredit/uniqredit/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/uniqredit/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/uniqredit/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /uniqreditrpc>
        ...
        # optional bypass uniqreditd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< uniqreditrpc:<password>
    </Location>
...
Restarting uniqreditd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Uniqredit-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new uniqredit-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include uniqredit-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix uniqredit-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of uniqredit-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override UNIQREDITD and UNIQREDITCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [uniqredit-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add UNIQREDITD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libuniqreditconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "UNIQ" (Ross Nicoll)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/uniqredit/).
