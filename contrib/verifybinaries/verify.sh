...
###   This script attempts to download the signature file SHA256SUMS.asc from uniqredit.org
...
WORKINGDIR="/tmp/uniqredit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://uniqredit.org/bin/"
VERSIONPREFIX="uniqredit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'uniqredit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Uniqredit Core binary release signing key installed?"
   fi
