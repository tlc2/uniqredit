...
The RPM spec file provided here is for Uniqredit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://uniqredit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Uniqredit git repository](https://github.com/uniqredit/uniqredit) but are not part of
...
Currently only `Source10` is used, for the example `uniqredit.conf` file.
...
in the uniqredit git repository. At present only an SVG version of the Uniqredit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the uniqredit
...
needed when 0.12.1 is released, a proper fix is already in the Uniqredit git
master branch.
...
idea to build Uniqredit Core against a static version of that library compiled
...
The `uniqredit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `uniqredit-server` package. At this point, most RPM based
...
The `uniqredit-server` package should have SELinux support. How to properly do
...
Uniqredit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/uniqredit/). He has been
packaging Uniqredit for Fedora at least since 2012.
...
4. I renamed the `uniqredit` package that contains the Qt GUI to `uniqredit-core` as
that appears to be how the general population refers to it, in contrast to
`uniqredit-xt` or `uniqredit-classic`. I wanted to make sure the general population
